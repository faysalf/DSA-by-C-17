...
void build(int node, int s, int e){
    ...
    tree[node] = min(tree[left_chi], tree[right_chi]);      //change this
}
ll query(int node, int s, int e, int i, int j){
    if (e<i or j<s) return inf;                           //return appropriate value
    ...
    int mid = (s+e) / 2;
    return min(query(left_chi,s,mid,i,j), query(right_chi,mid+1,e,i,j));    //change this
}
